...
using static ContainerBook.Core.Entities.MaerskResponse;

namespace ContainerBook.Services.Concrate
{
    public class MaerskService : IMaerskService
    {
        public async Task<BaseResponse<MaerskResponse>> GetTable(MaerskRequest request)
        {
            try
            {
                ...
                // Commodity ---------------------------------------------------------------------
                await page.WaitForTimeoutAsync(1000);
                await page.GetByPlaceholder("Type in minimum 2 characters").FillAsync(""); // Boş arama yaparak 
                await page.WaitForTimeoutAsync(1000);


                var element = page.Locator($"text='a'").First;
                await page.WaitForTimeoutAsync(1000);
                await element.ScrollIntoViewIfNeededAsync(); // Elementi görünür yap
                await page.WaitForTimeoutAsync(1000);
                ...
                //await article.Locator("span.hyperlink-button:has-text('Price breakdown & details')").ClickAsync();
                await page.WaitForTimeoutAsync(1000);
